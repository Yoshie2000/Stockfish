...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool givesCheck, improving, priorCapture, singularQuietLMR, shallowerProbcut;
    ...
    shallowerProbcut = ss->ttPv && tte->depth() >= depth + 1;
    ...
    if (   !PvNode
        ...
        && !(   ss->ttHit
             && tte->depth() >= depth - 3 - shallowerProbcut
             && ttValue != VALUE_NONE
             && ttValue < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= probCutBeta)
                    value = -search<NonPV>(pos, ss+1, -probCutBeta, -probCutBeta+1, depth - 4 - shallowerProbcut, !cutNode);
                ...
                if (value >= probCutBeta)
                {
                    ...
                    tte->save(posKey, value_to_tt(value, ss->ply), ss->ttPv, BOUND_LOWER, depth - 3 - shallowerProbcut, move, ss->staticEval);
                    return value;
                }
            }
