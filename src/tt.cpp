...
void TTEntry::save(
  Key k, Value v, bool pv, bool singular, Bound b, Depth d, Move m, Value ev, uint8_t generation8) {
        ...
        || relative_age(generation8))
    {
        ...
        genBound8 = uint8_t(generation8 | uint8_t(pv) << 2 | uint8_t(singular) << 3 | b);
        value16   = int16_t(v);
        eval16    = int16_t(ev);
    }
}
